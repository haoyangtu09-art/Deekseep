...
public final class ComposeHook {
    ...
    static void install(ClassLoader cl, Logger logger, ActGetter acts) {
        // A / B 已知会崩溃，保留但不启用（见类注释）。
        strategyA(cl, logger, acts);
        strategyB(cl, logger, acts);
        // C 是当前安全方案。
        strategyC(cl, logger, acts);
        probeU25B(cl, logger);
    }
    ...
    private static void strategyC(ClassLoader cl, final Logger logger, final ActGetter acts) {
        try {
            ...
            XposedBridge.hookMethod(bMethod, new XC_MethodHook() {
                ...
                protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
                    ...
                    try {
                        ...
                        // 别把 Deekseep 行自己当触发源。
                        if (INJECT_TITLE.equals(title)) return;

                        Object realRd5 = args[2];   // 真实 ViewModel/回调持有者
                        Object rd5ToUse = (realRd5 != null) ? realRd5 : rd5Proxy;
                        Object qt6 = args[3];       // NavController，复用真实值
                        Object rm5 = args[4];       // Modifier，复用真实值
    ...
    private static Object buildRd5Proxy(ClassLoader cl, final Logger logger, final ActGetter acts) {
        try {
